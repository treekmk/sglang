#!/usr/bin/env bash
# Launch SGLang serving openai/gpt-oss-120b on a single B200, TP=1, port 30001.
#
# gpt-oss-120b is mxfp4-quantized (~60 GB), fits 1× B200 with ~120 GB headroom
# for KV. The same flashinfer_mxfp4 MoE runner used for V4-Flash applies here
# (gpt-oss has 128 local experts, 4 active per token).
#
# Run AFTER `bash install_v4_flash.sh` (the venv set up there serves all models)
# and AFTER vacating the target GPU. Refuses to start if the port is taken.
...
LOG="$REPO_ROOT/serve_gpt_oss_120b.log"
PIDFILE="$REPO_ROOT/serve_gpt_oss_120b.pid"
HOST="${HOST:-0.0.0.0}"
PORT="${PORT:-30001}"
MODEL="${MODEL:-openai/gpt-oss-120b}"
MEM_FRACTION_STATIC="${MEM_FRACTION_STATIC:-0.85}"
GPUS="${CUDA_VISIBLE_DEVICES:-1}"
READY_DEADLINE_SEC="${READY_DEADLINE_SEC:-1800}"   # 30 min — gpt-oss is small
