#!/usr/bin/env bash
# Launch SGLang serving google/gemma-4-31B-it on a single B200, TP=1, port 30003.
#
# Gemma4-31B-it is a 31B dense BF16 model (~62 GB), fits 1× B200 with ~120 GB
# headroom for KV cache. 32 attention heads, 16 KV heads, head_dim 256.
#
# Run AFTER `bash install_v4_flash.sh` and AFTER vacating the target GPU.
...
LOG="$REPO_ROOT/serve_gemma4_31b.log"
PIDFILE="$REPO_ROOT/serve_gemma4_31b.pid"
HOST="${HOST:-0.0.0.0}"
PORT="${PORT:-30003}"
